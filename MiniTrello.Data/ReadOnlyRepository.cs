...
namespace MiniTrello.Data
{
    public class ReadOnlyRepository : IReadOnlyRepository
    {
        ...
        public IQueryable<T> UnQuery<T>(Expression<Func<T, bool>> expression) where T : class, IEntity
        {
            return _session.Query<T>().Where(expression).Where(x => x.IsArchived == true);
        }
    }
}